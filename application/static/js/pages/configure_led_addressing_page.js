...
const contextMouseMenuObject = MouseContextMenu(contextMenuItems);
const ledMouseMenuObject = MouseContextMenu(ledMenuItems);
//#endregion
...
async function save() {
    ...
    const result = await httpPostRequestJsonReturn("/update_ledstrip_leds", data, true);
...
function descendAddressing() {
    ...
    if (startAddress+1 - numberOfLedsInSegment < 0) {
        banners.show(TEXT_ERROR, TEXT_START_ADDRESS_TO_LOW, MESSAGE_TYPE_ERROR);
        return;
    }
...
function leftMouseUp(position) {
    ...
    if (selectedLedIndex !== -1) {
        ledMouseMenuObject.show({top: position.y, left: position.x});
...
function rightMouseUp(position) {
    ...
    if (!hasDragged && selectedSegmentIndex !== null) {
        ...
        contextMouseMenuObject.setMenuItems(contextMenuItems);
        //contextMouseMenuObject.show(position);
    } else {
        selectedSegmentIndex = null;
        selectedPointIndex = null;
    }
...
/*!
    ...
    @return array               Array with LED objects
*/
/******************************************************************************/
...
/*!
    ...
    @return integer             Distance between the point and segment
*/
/******************************************************************************/
...
function validateLedstripLedAddressing() {
    if (strip.leds.length == 0) {
        banners.show(TEXT_WARNING, TEXT_NO_OF_LEDS_CONFIGURED, MESSAGE_TYPE_WARNING);
        return false;
    }
    if (strip.leds.length > MAX_NUMBER_OF_LEDS) {
        banners.show(TEXT_ERROR, TEXT_MAXIMUM_NUMBER_OF_LEDS_REACHED, MESSAGE_TYPE_WARNING);
        return false;
    }
    ...
    if (numberOfLedsZero == strip.leds.length) {
        banners.show(TEXT_WARNING, TEXT_INVALID_ADDRESSING_NO_ADDRESSES, MESSAGE_TYPE_WARNING);
        return false;
    }
...
/*!
    ...
    @return integer             Highest address
*/
/******************************************************************************/
